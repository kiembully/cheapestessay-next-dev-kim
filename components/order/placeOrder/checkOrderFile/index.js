...
class Index extends Component {
    ...
    componentDidMount() {
        ...
        if (this.props.orderData) {
            ...
            this.setState({
                uploadedFiles: orderData.files && orderData.files.customer_files,
                writerFiles: orderData.files && orderData.files.writer_files.data,
    ...
    //     if ((this.props.listTab !== prevProps.listTab || prevProps.currentStep !== this.props.currentStep) && parseInt(this.props.currentStep) === 3) {
    //         // this.setFileData();
    ...
    //setFileData
    setFileData = () => {
        let fArray = localStorage.getItem('orderFileUpload');
        this.setState({ uploadedFiles: JSON.parse(fArray) });
    }
    ...
    changeFile = (e) => {
        ...
        var fArray = this.state.uploadedFiles ? this.state.uploadedFiles : [];

        for (var i = 0; i < files.length; i++) {
            let lastModified = files[i].lastModified || 0;
            let lastModifiedDate = (files[i].lastModifiedDate).toString() || null;
    ...
    render() {
        return (
            <>
                ...
                <div className="section-part leftSide">
                    <div className="orderFileDetails">
                        <div className="tabTitle d-flex align-content-center justify-content-between">
                            ...
                            <button className="file-input" onChange={this.uploadFiles}>
                                <input type="file" className="file-input__input" multiple={true} />
                            </button>
                        </div>
                        {
                            this.state.uploadedFiles && (this.state.uploadedFiles).length > 0 ?
                                <div className="files myFiles">
                                    <div className="row">
                                        {/* uploadedFiles */}
                                        {this.state.uploadedFiles && this.state.uploadedFiles.map((uf, index) => {

                                            //  let myObj = { lastModified: uf.lastModified, lastModifiedDate: uf.lastModifiedDate, name: uf.name, size: uf.size, size: uf.c, webkitRelativePath: uf.webkitRelativePath };
                                            // var d = new Date(uf.lastModified);
                                            // let full_date = d.getDate() + "/" + d.getMonth() + "/" + d.getFullYear();
                                                    ...
                                                    <div className="fileDetails d-flex justify-content-between">
                                                        <div className="icon d-flex">
                                                            ...
                                                            <div className="details">
                                                                <h4 className="name" title={uf.file_name1}>{(uf.file_name1).length > 25 ? (uf.file_name1).substring(0, 25) + "..." : uf.file_name1}</h4>
                                                                <span className="text">{moment(uf.date_time).format('DD MMM, YYYY  hh:mm A')}</span>
                                                            </div>
                                                        </div>
                                                        {/* onClick={() => this.openSelectedFile(myObj)} */}
                                                        <a className="view" target='_blank' href={uf.file_path}>
                                                        ...
                                                        <div className="removeFile" onClick={() => this.removeFile(uf.file_name)}>
                                                            <svg width={16} height={16} viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <div className="orderFileDetails">
                        ...
                        <div className="files">
                            {this.state.writerFiles && this.state.writerFiles.length > 0 ?
                                this.state.writerFiles.data.map(function (info, index) {
                                    return (
                                        <div className="col-md-6" key={index}>
                                            <div className="fileDetails d-flex justify-content-between" key={index}>
                                                <div className="icon d-flex">
                                                    <svg width={19} height={20} viewBox="0 0 19 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                        <path fillRule="evenodd" clipRule="evenodd" d="M12.9085 0C16.0525 0 18.1645 2.153 18.1645 5.357V14.553C18.1645 17.785 16.1175 19.887 12.9495 19.907L5.25651 19.91C2.11251 19.91 -0.000488281 17.757 -0.000488281 14.553V5.357C-0.000488281 2.124 2.04651 0.023 5.21451 0.004L12.9075 0H12.9085ZM12.9085 1.5L5.21951 1.504C2.89151 1.518 1.49951 2.958 1.49951 5.357V14.553C1.49951 16.968 2.90451 18.41 5.25551 18.41L12.9445 18.407C15.2725 18.393 16.6645 16.951 16.6645 14.553V5.357C16.6645 2.942 15.2605 1.5 12.9085 1.5ZM12.7156 13.4737C13.1296 13.4737 13.4656 13.8097 13.4656 14.2237C13.4656 14.6377 13.1296 14.9737 12.7156 14.9737H5.49561C5.08161 14.9737 4.74561 14.6377 4.74561 14.2237C4.74561 13.8097 5.08161 13.4737 5.49561 13.4737H12.7156ZM12.7156 9.2872C13.1296 9.2872 13.4656 9.6232 13.4656 10.0372C13.4656 10.4512 13.1296 10.7872 12.7156 10.7872H5.49561C5.08161 10.7872 4.74561 10.4512 4.74561 10.0372C4.74561 9.6232 5.08161 9.2872 5.49561 9.2872H12.7156ZM8.25031 5.1104C8.66431 5.1104 9.00031 5.4464 9.00031 5.8604C9.00031 6.2744 8.66431 6.6104 8.25031 6.6104H5.49531C5.08131 6.6104 4.74531 6.2744 4.74531 5.8604C4.74531 5.4464 5.08131 5.1104 5.49531 5.1104H8.25031Z" fill="black" />
                                                    </svg>
                                                ...
                                                <a className="view" target='_blank' href={info.file_path}>
                                                    <svg width={16} height={16} viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                        <g clipPath="url(#clip0)">
                ...
                {/* image modal */}
                <Modal isOpen={this.state.imageModal} toggle={() => this.setState({ imageModal: !this.state.imageModal })}>
                    <ModalBody>
